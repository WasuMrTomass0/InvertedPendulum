...
% Wybor sygnalow
x = data.P1_Data.time;
y = data.P1_Data.signals(2).values;
...
x = x - 120;
plot(x, y), grid on
title("Bujanie wahadla z wysokiej pozycji poczatkowej");
xlabel("Czas [s]");
ylabel("Kat wychylenia wahadla [rad]");
...
data = load("data/last_classes/stabilizacja_z_polozenia_poczatkowego_p30.mat");
...
% Wybor od ktorej probki
n_start = 920;
t = t(n_start:end) - 3087.4;
theta = theta(n_start:end);
x = x(n_start:end);
u = u(n_start:end);

% figure(1)
% subplot(3,1,1), plot(t, theta), grid on
% title("Bujanie wahadla z wysokiej pozycji poczatkowej");
% xlabel("Czas [s]");
% ylabel("Kat wychylenia wahadla [rad]");

figure();
subplot(3,1,1), plot(t(10:end), x(10:end)), grid on;
title("Stabilizacja wozka, \theta_0 \approx -\pi/12, x_0 \approx 0");
xlabel("Czas [s]"), ylabel("Polozenie wozka [m]");
subplot(3,1,2), plot(t, theta), grid on;
title("");
xlabel("Czas [s]"), ylabel("Wychylenie wahadla [rad]");
subplot(3,1,3), plot(t, u), grid on;
title("");
...
data = load("data/last_classes/wplyw_zaklocenia_delta_diraca_pchniecie_reka_p30.mat");
...
figure();
subplot(3,1,1), plot(t(10:end), x(10:end)), grid on;
title("Wplyw zakłócen na dzialanie ukladu");
xlabel("Czas [s]"), ylabel("Polozenie wozka [m]");
subplot(3,1,2), plot(t, theta), grid on;
title("");
xlabel("Czas [s]"), ylabel("Wychylenie wahadla [rad]");
